...
#[repr(C)]
pub union Rbt {
    Rbt: core::mem::ManuallyDrop<RbtGuts>,
}
...
#[repr(C)]
pub union Job {
    Job: core::mem::ManuallyDrop<JobGuts>,
}
...
#[repr(C)]
pub union Command {
    Command: core::mem::ManuallyDrop<CommandGuts>,
}
...
#[repr(C)]
pub union Tool {
    SystemTool: core::mem::ManuallyDrop<SystemTool>,
}
...
impl Rbt {
    ...
    /// (Always examine `.discriminant()` first to make sure this is the correct variant!)
    /// Panics in debug builds if the `.discriminant()` doesn't return `Rbt`.
    pub unsafe fn as_Rbt(&self) -> &RbtGuts {
        debug_assert_eq!(self.discriminant(), discriminant_Rbt::Rbt);
...
impl core::fmt::Debug for Rbt {
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        unsafe {
            match self.discriminant() {
                discriminant_Rbt::Rbt => f.debug_tuple("Rbt").field(&*self.Rbt).finish(),
            }
        }
    }
}
...
impl Job {
    ...
    /// (Always examine `.discriminant()` first to make sure this is the correct variant!)
    /// Panics in debug builds if the `.discriminant()` doesn't return `Job`.
    pub unsafe fn as_Job(&self) -> &JobGuts {
        debug_assert_eq!(self.discriminant(), discriminant_Job::Job);
...
impl Command {
    ...
    pub fn Command(arg0: CommandGuts) -> Self {
        let mut answer = Self {
    ...
    /// (Always examine `.discriminant()` first to make sure this is the correct variant!)
    /// Panics in debug builds if the `.discriminant()` doesn't return `Command`.
    pub unsafe fn as_Command(&self) -> &CommandGuts {
        debug_assert_eq!(self.discriminant(), discriminant_Command::Command);
...
impl Tool {
    ...
    pub fn SystemTool(arg0: SystemTool) -> Self {
        let mut answer = Self {
    ...
    /// (Always examine `.discriminant()` first to make sure this is the correct variant!)
    /// Panics in debug builds if the `.discriminant()` doesn't return `SystemTool`.
    pub unsafe fn as_SystemTool(&self) -> &SystemTool {
        debug_assert_eq!(self.discriminant(), discriminant_Tool::SystemTool);
